...
class AXEngine:
    ...
    def save_inspection_buffer(self, path=None):
        """Save recorded buffer to JSON file in ~/.claude/skills/ax-executor/recorded/"""
        import time as _t

        # Default: save to recorded folder with timestamp
        if path is None:
            recorded_dir = os.path.expanduser("~/.claude/skills/ax-executor/recorded")
            os.makedirs(recorded_dir, exist_ok=True)
            timestamp = _t.strftime("%Y%m%d_%H%M%S")
            path = os.path.join(recorded_dir, f"recording_{timestamp}.json")
    ...
    def save_search_result(self, search_label, best_info, best_score, app_name, pid, filename=None):
        """Save search result to recorded folder as JSON"""
        import time as _t
        ...
        try:
            ...
            if filename is None:
                timestamp = _t.strftime("%Y%m%d_%H%M%S")
                safe_label = search_label.replace(" ", "_").replace("/", "_")
                filename = f"search_{safe_label}_{timestamp}.json"
            ...
            record = {
                ...
                "app_name": app_name,
                "app_pid": pid,
                "found": best_info is not None,
                "score": best_score if best_score else 0.0,
                "element_info": best_info if best_info else {}
            }
            ...
            print(f"[ERROR] Could not save search result: {e}")
            return None
