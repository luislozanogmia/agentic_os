...
Options:
    ...
    --type TEXT       Paste TEXT after a successful click (or dry run if no click)
    --type-file PATH  Paste contents of PATH after a successful click
    --no-enter        Do not press Enter after pasting (default is to press Enter when --type/--type-file is used)
...
class AXExecutorCLI:
    ...
    def parse_args(self, argv):
        ...
        while i < len(argv):
            ...
            if arg == '--index' and i + 1 < len(argv):
                ...
                args['type_text'] = argv[i + 1]
                i += 1
            elif arg == '--type-file' and i + 1 < len(argv):
                args['type_file'] = argv[i + 1]
                i += 1
            elif arg == '--no-enter':
    ...
    def run(self, argv):
        ...
        # Optional: type/paste after a successful click
        to_type = None
        if args.get('type_file'):
            try:
                with open(args['type_file'], 'r', encoding='utf-8') as f:
                    to_type = f.read()
            except Exception as e:
                print(f"[ERROR] Could not read --type-file: {e}")
                return False
        elif args.get('type_text'):
            to_type = args['type_text']

        if to_type is not None:
            if not args['do_click']:
                print("[WARN] --type was provided but no click was performed. Add --click/-c or --click-if-match to focus the input first.")
            if success:
                # Paste via clipboard for reliability
                p = subprocess.Popen(['pbcopy'], stdin=subprocess.PIPE)
                p.communicate(to_type.encode('utf-8'))
                time.sleep(0.05)
                subprocess.run(
...
Options:
  ...
  --diag              Show detailed L1/L2/L3 diagnostics
  --escalate          Retry up to 3 times (L7 Vision)
  --type TEXT        Paste TEXT after a successful click
  --type-file PATH   Paste file contents after a successful click
  --no-enter         Do not press Enter after pasting

Examples:
  python test_ax_execute.py --index 0
  python test_ax_execute.py --click-if-match --escalate
  python test_ax_execute.py --click --diag
  python test_ax_execute.py --index 0 --click --type "wake up claude check your results"
